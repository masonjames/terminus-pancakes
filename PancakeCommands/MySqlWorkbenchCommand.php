...
class MySqlWorkbenchCommand extends PancakesCommand {
   ...
   * {@inheritdoc}
   */
  public $aliases = ['mysql-workbench'];
  ...
  public function pancakes($args, $assoc_args) {
    $domain = $this->environment->get('name') . '-' . $this->site->get('name') . '.pantheon.io';
    ...
    $connections_file = "{$this->app_location}connections.xml";
  ...
  protected function validate($args, $assoc_args) {
    ...
    switch ($os) {
      case 'DAR':
        $workbench_cmd = '/Applications/MySQLWorkbench.app/Contents/MacOS/MySQLWorkbench';
        $workbench_home = getenv('HOME') . '/Library/Application Support/MySQL/Workbench/';
        break;
      case 'LIN';
        $workbench_cmd = 'mysql-workbench';
        $workbench_home = getenv('HOME') . '/.mysql/workbench/';
        break;
      case 'WIN':
        $workbench_cmd = 'MySQLWorkbench';
    ...
    if ($this->which($workbench_cmd)) {
      $this->app_home_location = $workbench_home;
      $this->app_location = $workbench_cmd;
      return TRUE;
    }
